...
// Rename default skim cores to standard names
    ...
    mObj.RenameCores({CurrentNames: {        "time","toll_auto (skim)","Length (skim)"}, 
                          NewNames: {mode + "_time",    mode + "_toll", mode + "_dist"}})
    return(ok)
endmacro
...
// Combine skims into a single file by tod
    ...
    mtrk_skim = out_mtx_dir + "hwy_mtrk_"+tod+".mtx"
    htrk_skim = out_mtx_dir + "hwy_htrk_"+tod+".mtx"
    comb_skim = out_mtx_dir + "truck_skims_"+tod+".mtx"
